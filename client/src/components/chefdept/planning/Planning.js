...
const Planning = () => {
  const ShowStudents = (value) => {
    return (
      <ul>
        <li>{value[0].enseignant1}</li>
        <li>{value[1].enseignant2}</li>
      </ul>
    );
  };
  ...
  const columns = [
    ...
    {
      field: "planifier",
      headerName: "Planifier",
      width: 150,
      renderCell: (params) => (
        <div>
          <Link to={"newPlanning/" + params.value}>
            {" "}
